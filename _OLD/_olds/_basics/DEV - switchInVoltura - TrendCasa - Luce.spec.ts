...
const subFolder = "DEV_SWITCH_IN_VOLTURA_LUCE";
const path = `tests/screens/${subFolder}`;
const screenName = subFolder.toLowerCase() + "_";

//SW -> trend casa > luce
const startUrl =
  "http://localhost:4200/configura-offerta?codiceProdotto=BASE_LTCASAV-GTCASAV&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&salesProcess=SWITCH_IN_TRANSFER&mocks&dev";

test("DEV - Switch In con Voltura / Trend Casa / Luce", async ({ page }) => {
  ...
  const steps: {
    ...
    {
      title: ["Inserisci il tuo codice pod", "Inserisci il punto di fornitura"],
      action: async (page) => {
        await page.waitForTimeout(WAIT.SCREENSHOT);
        await autoFillForm(page, path, screenName);
        await nextStepButton(page, true, path, screenName);
