...
const subFolder = "PP_SWITCH_IN_VOLTURA_LUCE";
const path = `tests/screens/${subFolder}`;
const screenName = subFolder.toLowerCase() + "_";

//SW -> trend casa > luce
const startUrl =
  "https://pp.eniplenitude.com/configura-offerta?codiceProdotto=BASE_LTCASAV-GTCASAV&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&salesProcess=SWITCH_IN_TRANSFER&dev=true";

test("PP - Switch In con Voltura / Trend Casa / Luce", async ({ page }) => {
