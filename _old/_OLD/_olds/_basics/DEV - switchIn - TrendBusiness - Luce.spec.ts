...
//SW -> trendbusiness > luce
const startUrl =
  "http://localhost:4200/configura-offerta?codiceProdotto=BASE_TNDVL-TNDVG&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&salesProcess=SWITCH_IN&mocks&dev";

test("DEV - Switch In / Trend Business / Luce", async ({ page }) => {
  ...
  const steps: {
    ...
    {
      title: [
        "Chi è il rappresentante legale dell’azienda?",
        "A chi appartiene l’immobile",
      ],
      action: async (page) => {
        await page.waitForTimeout(WAIT.SCREENSHOT);
        await autoFillForm(page, path, screenName);
    ...
    {
      title: [
        "Vuoi accorciare i tempi di attivazione? Scegli l’esecuzione anticipata!",
      ],
      action: async (page) => {
        await page.waitForTimeout(WAIT.SCREENSHOT);
        await autoFillForm(page, path, screenName);
