...
const startUrl = `http://localhost:4200/configura-offerta?codiceProdotto=BASE_LFIXATIV-GFIXATIV&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&salesProcess=SWITCH_IN&mocks=&dev=&segment=res`

test(`SWITCH FIXA LUCE BOLLETTINO`, async ({ page }) => {
  // Lista Step
  ...
  const subFolder = 'SWITCH_FIXA_LUCE_BOLLETTINO';
