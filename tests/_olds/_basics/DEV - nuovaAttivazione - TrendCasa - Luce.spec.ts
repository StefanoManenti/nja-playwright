...
const subFolder = "DEV_NA_TRENDCASA_LUCE";
const path = `tests/screens/${subFolder}`;
const screenName = subFolder.toLowerCase() + "_";

//NA -> trendcasa > luce
const startUrl =
  "http://localhost:4200/configura-offerta?codiceProdotto=BASE_LTCASAV-GTCASAV&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&salesProcess=NEW_ACTIVATION&mocks&dev";

test("DEV - Nuova Attivazione / Trend Casa / Luce", async ({ page }) => {
  ...
  const steps: {
    ...
    {
      title: ["Qual è il tuo consumo annuale di energia elettrica"],
      action: async (page) => {
        await page.waitForTimeout(WAIT.SCREENSHOT);
        await autoFillForm(page, path, screenName);
        await nextStepButton(page, true, path, screenName);
