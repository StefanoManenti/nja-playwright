...
const startUrl =
  "http://localhost:4200/configura-offerta?codiceProdotto=BASE_TNDVL-TNDVG&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&mocks&dev";

test("Trend Business / Luce / Cambio fornitore", async ({ page }) => {
  const subFolder = "DEVB3_TB_LUCE_CAMBIO_FORNITORE";
  ...
  const steps: {
    ...
    {
      title: ["Trend Business Luce", "Prossimi passi"],
      action: async (page) => {
        await nextStepButton(page, true, path, screenName);
      },
    },
    ...
    },
    {
      title: [
        ...
        "Dove vuoi attivare la fornitura?",
      ],
      action: async (page) => {
...
test("Trend Business / Luce / Cambio intestatario", async ({ page }) => {
  const subFolder = "DEVB3_TB_LUCE_CAMBIO_INTESTATARIO";
  const path = `tests/screens/${subFolder}`;
  const screenName = subFolder.toLowerCase() + "_";
