...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH IN FIXA DUAL - iPhone';
...
test(`SWITCH IN FIXA DUAL - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH IN FIXA DUAL - iPhone`);
  ...
  const subFolder = 'SWITCH_IN_FIXA_DUAL_IPHONE';
