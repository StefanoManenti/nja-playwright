...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'PP - VOLTURA GAS (mortis causa) - iPhone';
...
test(`PP - VOLTURA GAS (mortis causa) - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – PP - VOLTURA GAS (mortis causa) - iPhone`);
  ...
  const subFolder = 'PP_VOLTURA_GAS_MORTIS_CAUSA_IPHONE';
