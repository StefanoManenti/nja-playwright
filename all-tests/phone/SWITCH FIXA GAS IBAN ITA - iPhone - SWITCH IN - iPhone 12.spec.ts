...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH FIXA GAS IBAN ITA - iPhone';
...
test(`SWITCH FIXA GAS IBAN ITA - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH FIXA GAS IBAN ITA - iPhone`);
  ...
  const subFolder = 'SWITCH_FIXA_GAS_IBAN_ITA_IPHONE';
