...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'NUOVA ATTIVAZIONE GAS FIXA (subentro) - iPhone';
...
test(`NUOVA ATTIVAZIONE GAS FIXA (subentro) - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – NUOVA ATTIVAZIONE GAS FIXA (subentro) - iPhone`);
  ...
  const subFolder = 'NUOVA_ATTIVAZIONE_GAS_FIXA_SUBENTRO_IPHONE';
