...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH IN CON VOLTURA LUCE - iPhone';
...
test(`SWITCH IN CON VOLTURA LUCE - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH IN CON VOLTURA LUCE - iPhone`);
  ...
  const subFolder = 'SWITCH_IN_CON_VOLTURA_LUCE_IPHONE';
