...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'PP - ADOTTA UN PANNELLO - iPhone';
...
test(`PP - ADOTTA UN PANNELLO - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – PP - ADOTTA UN PANNELLO - iPhone`);
  ...
  const subFolder = 'PP_ADOTTA_UN_PANNELLO_IPHONE';
