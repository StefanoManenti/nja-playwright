...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'PP - CAMBIO PRODOTTO LUCE - iPhone';
...
test(`PP - CAMBIO PRODOTTO LUCE - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – PP - CAMBIO PRODOTTO LUCE - iPhone`);
  ...
  const subFolder = 'PP_CAMBIO_PRODOTTO_LUCE_IPHONE';
