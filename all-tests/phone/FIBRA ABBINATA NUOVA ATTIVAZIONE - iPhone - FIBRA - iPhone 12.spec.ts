...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'FIBRA ABBINATA NUOVA ATTIVAZIONE - iPhone';
...
test(`FIBRA ABBINATA NUOVA ATTIVAZIONE - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – FIBRA ABBINATA NUOVA ATTIVAZIONE - iPhone`);
  ...
  const subFolder = 'FIBRA_ABBINATA_NUOVA_ATTIVAZIONE_IPHONE';
