...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH FIXA LUCE BOLLETTINO - iPhone';
...
test(`SWITCH FIXA LUCE BOLLETTINO - iPhone`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH FIXA LUCE BOLLETTINO - iPhone`);
  ...
  const subFolder = 'SWITCH_FIXA_LUCE_BOLLETTINO_IPHONE';
