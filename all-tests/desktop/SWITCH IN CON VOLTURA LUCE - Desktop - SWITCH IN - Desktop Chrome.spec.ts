...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH IN CON VOLTURA LUCE - Desktop';
...
test(`SWITCH IN CON VOLTURA LUCE - Desktop`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH IN CON VOLTURA LUCE - Desktop`);
  ...
  const subFolder = 'SWITCH_IN_CON_VOLTURA_LUCE_DESKTOP';
