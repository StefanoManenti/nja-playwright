...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH FIXA LUCE BOLLETTINO - Desktop';
...
test(`SWITCH FIXA LUCE BOLLETTINO - Desktop`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH FIXA LUCE BOLLETTINO - Desktop`);
  ...
  const subFolder = 'SWITCH_FIXA_LUCE_BOLLETTINO_DESKTOP';
