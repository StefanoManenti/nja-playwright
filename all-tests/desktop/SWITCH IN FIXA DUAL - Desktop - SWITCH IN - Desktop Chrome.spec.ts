...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH IN FIXA DUAL - Desktop';
...
test(`SWITCH IN FIXA DUAL - Desktop`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH IN FIXA DUAL - Desktop`);
  ...
  const subFolder = 'SWITCH_IN_FIXA_DUAL_DESKTOP';
