...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'PP - CAMBIO PRODOTTO GAS - Desktop';
...
test(`PP - CAMBIO PRODOTTO GAS - Desktop`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – PP - CAMBIO PRODOTTO GAS - Desktop`);
  ...
  const subFolder = 'PP_CAMBIO_PRODOTTO_GAS_DESKTOP';
