...
const queryString = `codiceProdotto=BASE_PANNELLO&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&salesProcess=AGGIUNGI_PANNELLO&panel=`;
...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'PP - ADOTTA UN PANNELLO - iPad';
...
test(`PP - ADOTTA UN PANNELLO - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – PP - ADOTTA UN PANNELLO - iPad`);
  ...
  const subFolder = 'PP_ADOTTA_UN_PANNELLO_IPAD';
