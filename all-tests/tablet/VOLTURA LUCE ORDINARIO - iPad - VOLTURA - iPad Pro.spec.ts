...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'VOLTURA LUCE ORDINARIO - iPad';
...
test(`VOLTURA LUCE ORDINARIO - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – VOLTURA LUCE ORDINARIO - iPad`);
  ...
  const subFolder = 'VOLTURA_LUCE_ORDINARIO_IPAD';
