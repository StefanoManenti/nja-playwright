...
const queryString = `codiceProdotto=BASE_LFIXATIV-GFIXATIV&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=gas&salesProcess=NEW_ACTIVATION`;
...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'NUOVA ATTIVAZIONE GAS FIXA (subentro) - iPad';
    const deviceArg = 'iPad Pro';
...
test(`NUOVA ATTIVAZIONE GAS FIXA (subentro) - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – NUOVA ATTIVAZIONE GAS FIXA (subentro) - iPad`);
  ...
  const subFolder = 'NUOVA_ATTIVAZIONE_GAS_FIXA_SUBENTRO_IPAD';
