...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'PP - CAMBIO PRODOTTO DUAL - iPad';
...
test(`PP - CAMBIO PRODOTTO DUAL - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – PP - CAMBIO PRODOTTO DUAL - iPad`);
  ...
  const subFolder = 'PP_CAMBIO_PRODOTTO_DUAL_IPAD';
