...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'VOLTURA DUAL IBAN STRANIERO - iPad';
...
test(`VOLTURA DUAL IBAN STRANIERO - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – VOLTURA DUAL IBAN STRANIERO - iPad`);
  ...
  const subFolder = 'VOLTURA_DUAL_IBAN_STRANIERO_IPAD';
