...
const queryString = `codiceProdotto=BASE_LFIXATIV-GFIXATIV&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&commodity=luce&salesProcess=SWITCH_IN&segment=res`;
...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SWITCH FIXA LUCE BOLLETTINO - iPad';
...
test(`SWITCH FIXA LUCE BOLLETTINO - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SWITCH FIXA LUCE BOLLETTINO - iPad`);
  ...
  const subFolder = 'SWITCH_FIXA_LUCE_BOLLETTINO_IPAD';
