...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'SUBENTRO LUCE TREND - iPad';
...
test(`SUBENTRO LUCE TREND - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – SUBENTRO LUCE TREND - iPad`);
  ...
  const subFolder = 'SUBENTRO_LUCE_TREND_IPAD';
