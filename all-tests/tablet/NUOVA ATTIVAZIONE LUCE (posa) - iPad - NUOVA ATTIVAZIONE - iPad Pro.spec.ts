...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'NUOVA ATTIVAZIONE LUCE (posa) - iPad';
...
test(`NUOVA ATTIVAZIONE LUCE (posa) - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – NUOVA ATTIVAZIONE LUCE (posa) - iPad`);
  ...
  const subFolder = 'NUOVA_ATTIVAZIONE_LUCE_POSA_IPAD';
