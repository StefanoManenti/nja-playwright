...
const queryString = `codiceProdotto=BASE_BB&codiceCanale=CWEB3EGP&codiceTpCanale=WB&direct-debit=true&bill-type=digitale&salesProcess=BROADBAND&fiber=BASE_BB&egonid=380100013650019`;
...
async function generateExcelReport(logFilePath: string, testId: number, runId: number, testName: string) {
  return new Promise<void>((resolve, reject) => {
    ...
    const testNameArg = 'FIBRA STANDALONE MIGRAZIONE - iPad';
...
test(`FIBRA STANDALONE MIGRAZIONE - iPad`, async ({ page }) => {
  const startTime = new Date().toISOString();
  logs.push(`[test-start] ${startTime} – FIBRA STANDALONE MIGRAZIONE - iPad`);
  ...
  const subFolder = 'FIBRA_STANDALONE_MIGRAZIONE_IPAD';
